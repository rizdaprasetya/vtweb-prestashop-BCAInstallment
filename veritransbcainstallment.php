...
class VeritransBcaInstallment extends PaymentModule
{
	private $_html = '';
	...
	{
		$this->name = 'veritransbcainstallment';
		...
		$this->config_keys = array(			
			...
			'VCI_KURS',
			'VCI_CONVENIENCE_FEE',
			'VCI_PAYMENT_SUCCESS_STATUS_MAP',
			'VCI_PAYMENT_FAILURE_STATUS_MAP',
			'VCI_PAYMENT_CHALLENGE_STATUS_MAP',
			'VCI_ENVIRONMENT',
			'VCI_SANITIZED',
			'VCI_ENABLE_INSTALLMENT',
			// 'VCI_ENABLED_BNI_INSTALLMENT',
			// 'VCI_ENABLED_OFFLINE_INSTALLMENT',
			'VCI_INSTALLMENTS_BNI',
			'VCI_INSTALLMENTS_TERM',
			'VCI_BIN_FILTER',
			'VCI_DISPLAY_NAME',
			'VCI_AMOUNT_THRESHOLD'
			);

		foreach (array('BNI', 'MANDIRI') as $bank) {
			foreach (array(3, 6, 12) as $months) {
				array_push($this->config_keys, 'VCI_INSTALLMENTS_' . $bank . '_' . $months);
			}
		}
		...
		if (isset($config['VCI_KURS']))
			$this->veritrans_kurs = $config['VCI_KURS'];
		else
			Configuration::set('VCI_KURS', 10000);

		if (!isset($config['VCI_DISPLAY_NAME']))
			Configuration::set('VCI_DISPLAY_NAME', "Credit Card BCA Installment");

		if (!isset($config['VCI_AMOUNT_THRESHOLD']) || strlen($config['VCI_AMOUNT_THRESHOLD']) < 1)
			Configuration::set('VCI_AMOUNT_THRESHOLD', "500000");
		
		Configuration::set('VCI_API_VERSION', 2);
		Configuration::set('VCI_PAYMENT_TYPE','vtweb');

		if (!isset($config['VCI_SANITIZED']))
			Configuration::set('VCI_SANITIZED', 0);	
		parent::__construct();

		$this->displayName = $this->l('Veritrans BCA Installment');
		$this->description = $this->l('Accept BCA installment payments for your products via Veritrans.');
		$this->confirmUninstall = $this->l('Are you sure about uninstalling Veritrans BCA installment?');
	...
	public function install()
	{
		...
		$order_state->module_name = 'veritransbcainstallment';
		...
		Configuration::updateValue('VCI_API_VERSION', 2);
		...
		include_once(_PS_MODULE_DIR_ . '/' . $this->name . '/vtbcainstallment_install.php');
		$vtbcainstallment_install = new VeritransBcaInstallmentInstall();
		$vtbcainstallment_install->createTable();
	...
	private function _postValidation()
	{
		...
		{
			if (Tools::getValue('VCI_API_VERSION') == 2)
			{
				if (!Tools::getValue('VCI_SERVER_KEY'))
					$this->_postErrors[] = $this->l('Server Key is required.');
			} 

			// validate conversion rate existence
			if (!Currency::exists('IDR', null) && !Tools::getValue('VCI_KURS'))
			{
	...
	private function _displayVeritransBcaInstallment()
	{
	...
	private function _displayVeritransBcaInstallmentOld()
	{
		$this->_html .= '<img src="../modules/veritransbcainstallment/Veritrans.png" style="float:left; margin-right:15px;"><b>'.$this->l('This module  BCA installment payment via veritrans.').'</b><br/><br/>
		...
		$fields_form = array(
			'form' => array(
				...
				'input' => array(
					...
					array(
						'type' => 'select',
						'label' => 'Enable BCA Installments',
						'name' => 'VCI_ENABLE_INSTALLMENT',						
						'options' => array(
					...
					// 	'name' => 'VCI_ENABLED_OFFLINE_INSTALLMENT',						
					// 	'is_bool' => true,
					// 	'values' => array(
					// 		array(
					// 			'id' => 'VCI_ENABLED_OFFLINE_INSTALLMENT_on',
					// 			'value' => 1,
					// 			'label' => 'Yes'
					// 			),
					// 		array(
					// 			'id' => 'VCI_ENABLED_OFFLINE_INSTALLMENT_off',
					// 			'value' => 0,
					// 			'label' => 'No'
					// 			)
					// 		),
					// 	//'class' => 'VCI_ENABLED_OFFLINE_INSTALLMENT'
					// 	),
					array(
						'type' => 'text',
						'label' => 'BCA Installment terms',
						'name' => 'VCI_INSTALLMENTS_TERM',
						'desc' => 'Insert installment terms in months, separate using coma (,) for multiple value. e.g: 3,6,12',
						//'class' => 'v1_vtweb_settings sensitive'\
						'class' => 'VCI_INSTALLMENTS_TERM'	
						),
					array(
						'type' => 'text',
						'label' => 'BIN Filter Number',
						'name' => 'VCI_BIN_FILTER',
						'desc' => 'Insert BIN Number to allow only payment from certain Bank/Credit Card Issuer, separate using coma (,) for multiple value. e.g: 48111111,3111,5,bni,mandiri',
						//'class' => 'v1_vtweb_settings sensitive'\
						'class' => 'VCI_BIN_FILTER'	
						),
					array(
						'type' => 'text',
						'label' => 'Payment Button Display Title',
						// 'required' => true,	
						'name' => 'VCI_DISPLAY_NAME',
				...
				/*	array(
					...
					array(
						...
						'name' => 'VCI_PAYMENT_SUCCESS_STATUS_MAP',
						'required' => true,	
						'options' => array(
							'query' => $order_states,
							'id' => 'id_option',
					...
					array(
						...
						'name' => 'VCI_PAYMENT_FAILURE_STATUS_MAP',
						'required' => true,
						'options' => array(
							'query' => $order_states,
							'id' => 'id_option',
					...
					array(
						...
						'name' => 'VCI_PAYMENT_CHALLENGE_STATUS_MAP',
						'required' => true,
						'options' => array(
							'query' => $order_states,
							'id' => 'id_option',
	...
	private function _displayFormOld()
	{
		...
		$this->context->smarty->assign(array(
			...
			'api_version' => htmlentities(Configuration::get('VCI_API_VERSION'), ENT_COMPAT, 'UTF-8'),
			'api_versions' => array(1 => 'v1', 2 => 'v2'),
			//'payment_type' => htmlentities(Configuration::get('VCI_PAYMENT_TYPE'), ENT_COMPAT, 'UTF-8'),
			//'payment_types' => array('vtweb' => 'VT-Web', 'vtdirect' => 'VT-Direct'),
			// 'client_key' => htmlentities(Configuration::get('VCI_CLIENT_KEY'), ENT_COMPAT, 'UTF-8'),
			'server_key' => htmlentities(Configuration::get('VCI_SERVER_KEY'), ENT_COMPAT, 'UTF-8'),
			'environments' => array(false => 'Development', true => 'Production'),
			'environment' => htmlentities(Configuration::get('VCI_ENVIRONMENT'), ENT_COMPAT, 'UTF-8'),
			// 'enable_3d_secure' => htmlentities(Configuration::get('VCI_3D_SECURE'), ENT_COMPAT, 'UTF-8'),
			// 'enable_sanitized' => htmlentities(Configuration::get('VCI_SANITIZED'), ENT_COMPAT, 'UTF-8'),
			...
			'payment_success_status_map' => htmlentities(Configuration::get('VCI_PAYMENT_SUCCESS_STATUS_MAP'), ENT_COMPAT, 'UTF-8'),
			'payment_challenge_status_map' => htmlentities(Configuration::get('VCI_PAYMENT_CHALLENGE_STATUS_MAP'), ENT_COMPAT, 'UTF-8'),
			'payment_failure_status_map' => htmlentities(Configuration::get('VCI_PAYMENT_FAILURE_STATUS_MAP'), ENT_COMPAT, 'UTF-8'),
			'kurs' => htmlentities(Configuration::get('VCI_KURS', $this->veritrans_kurs), ENT_COMPAT, 'UTF-8'),
			//'kurs' => htmlentities(Configuration::get('VCI_INSTALLMENTS_BNI', ENT_COMPAT, 'UTF-8'),
			'convenience_fee' => htmlentities(Configuration::get('VCI_CONVENIENCE_FEE', $this->veritrans_convenience_fee), ENT_COMPAT, 'UTF-8'),
	...
	public function getContent()
	{
		...
		$this->_displayVeritransBcaInstallment();
		$this->_displayForm();
	...
	public function hookDisplayPayment($params)
	{
		...
		if($num_product == 1 && Configuration::get('VCI_ENABLE_INSTALLMENT') == 'certain_product'){
		...
		if (Configuration::get('VCI_ENABLE_INSTALLMENT') == 'all_product' /*&& $num_product == 1*/) 
			$product = 'installment';
		if (Configuration::get('VCI_ENABLE_INSTALLMENT') == 'off' /*|| !Configuration::get('VCI_ENABLED_OFFLINE_INSTALLMENT')*/) 
		...
		if($cart->getOrderTotal() <= Configuration::get('VCI_AMOUNT_THRESHOLD')){
			$product = 'below_threshold';
		}

		// Check if display title form is filled by user, else default name will be used
		$displayname = Configuration::get('VCI_DISPLAY_NAME');
		if(strlen($displayname) <= 1){
			$displayname = "BCA Credit Card Installment";
		}

		$this->context->smarty->assign(array(
			'displayname' => $displayname,
			'product'=> $product,
			'amount_threshold' => Configuration::get('VCI_AMOUNT_THRESHOLD'),
			'cart' => $cart,
			'this_path' => $this->_path,
			'this_path_ssl' => Tools::getShopDomainSsl(true, true).__PS_BASE_URI__.'modules/'.$this->name.'/'
		));
	...
	public function execPayment($cart)
	{
		...
		$smarty->assign(array(
			'payment_type' => Configuration::get('VCI_PAYMENT_TYPE'),
			'api_version' => Configuration::get('VCI_API_VERSION'),
			'error_message' => '',
			'link' => $link,
...
//
	public function getTermInstallment($name_bank){
		$ans = array();
		foreach ($this->config_keys as $key) {
			if ( (strpos($key, 'VCI_INSTALLMENTS_' . $name_bank ) !== FALSE) && (Configuration::get($key) == 'on') ){
				
				$term = Configuration::get('VCI_INSTALLMENTS_'.$name_bank);
	...
	public function execValidation($cart)
	{
		...
		foreach (Module::getPaymentModules() as $module)
			if ($module['name'] == 'veritransbcainstallment')
			{
				$authorized = true;
				break;
			}
		...
		//SETUP
		Veritrans_Config::$serverKey = Configuration::get('VCI_SERVER_KEY');
		Veritrans_Config::$isProduction = Configuration::get('VCI_ENVIRONMENT') == 'production' ? true : false;
	...
	// if (Configuration::get('VCI_SANITIZED') == 'on' || Configuration::get('VCI_SANITIZED') == 1)
			Veritrans_Config::$isSanitized = false;
		
		//error_log('sanitized '.Configuration::get('VCI_SANITIZED'));
		...
		if ($cart_currency->iso_code != 'IDR')
		{
			...
			{
			  // use rate
			  $conversion_func = function($input) { return $input * intval(Configuration::get('VCI_KURS')); };
			}
		...
		// $isBniInstallment = Configuration::get('VCI_ENABLED_BNI_INSTALLMENT') == 1;
		// $isOfflineInstallment = Configuration::get('VCI_ENABLED_OFFLINE_INSTALLMENT') == 1;
		$isBcaInstallment = true;
		$isBniInstallment = false;
		$warning_redirect = false;
		$fullPayment = true;

		$installment_type_val = Configuration::get('VCI_ENABLE_INSTALLMENT');
		$param_required;
		$offline_term = array();
		switch ($installment_type_val) {
			case 'all_product':
				...
				// 	$a = Configuration::get('VCI_INSTALLMENTS_BNI');
				// 	$term = explode(',',$a);
				// 	$bni_term = $term;
				...
				if ($isBcaInstallment){
					...
					// $offline_term =	$this->getTermInstallment('MANDIRI');
					
					$a = Configuration::get('VCI_INSTALLMENTS_TERM');
					$term = explode(',',$a);
					$offline_term = $term;
					
					//error_log($offline_term,true);
					//error_log(print_r($offline_term,true));
				}				
				
				$param_installment = new stdClass();
				// if ($isBniInstallment){
				// 	$param_installment['bni'] = $bni_term;
				// }

				// if ($isOfflineInstallment){
				// 	$param_installment['mandiri'] = $offline_term;
				// }
				$param_required = "true";
				$fullPayment = false;
				break;
			case 'certain_product':
				...
				if($num_product == 1){
					...
					foreach($attr_product as $att){
						...
						if(strtolower($att_arr[0]) == 'installment' && strtolower($att_arr[1]) == 'bca'){
							$fullPayment = false;
							$param_installment = new stdClass();
							$offline_term = array($att_arr[2]);
						} 						
					}
				} else {
		...
		// get bin filter config
		$a = Configuration::get('VCI_BIN_FILTER');
		$bin_filter = explode(',',$a);
	...
	//error_log($param_installment,true);
		...
		if ($gross_amount < Configuration::get('VCI_AMOUNT_THRESHOLD')){
			$warning_redirect = true;
			$keys['message'] = Configuration::get('VCI_AMOUNT_THRESHOLD');
		}
		...
		if (Configuration::get('VCI_API_VERSION') == 2 && Configuration::get('VCI_PAYMENT_TYPE') != 'vtdirect') //transaksi https://github.com/veritrans/veritrans-php/blob/vtweb-2/examples/v2/vt_web/checkout_process.php line 77
		{						
			try {
	...
	public function execNotification()
	{
		...
		//{
			...
			if ($veritrans_notification->transaction_status == 'capture')				
		    {
		     	...
		     	{
		       		$history->changeIdOrderState(Configuration::get('VCI_PAYMENT_SUCCESS_STATUS_MAP'), $order_id_notif);
		       		echo 'Valid success notification accepted.';
		       	}
		       	else if ($veritrans_notification->fraud_status== 'challenge')
		     	{
		       		$history->changeIdOrderState(Configuration::get('VCI_PAYMENT_CHALLENGE_STATUS_MAP'), $order_id_notif);
		     	...
		     	$history->changeIdOrderState(Configuration::get('VCI_PAYMENT_FAILURE_STATUS_MAP'), $order_id_notif);
		       	echo 'Pending notification accepted.';
